...
postername tinytext,
uid int(10) unsigned,
usefeeddate tinyint(4) NOT NULL default '0',
...
function rssfeedposter_uninstall()
{
	...
	$db->query("DELETE FROM ".TABLE_PREFIX."tasks WHERE `file` = 'rssfeedposter'
...
function rssfeedposter_admin()
{
	...
	if ($mybb->input['action'] == 'add' || $mybb->input['action'] == 'add2')
	{
		...
		$table->construct_cell($lang->rssfeedposter_usefeeddate);
		$table->construct_cell('<input type="checkbox" name="usefeeddate" ' . ($usefeeddate ? ' checked="checked" ' : ''). ' />');
		$table->construct_row();
